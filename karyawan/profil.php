...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'update_profile') {
        ...
        if (empty($new_nama)) {
            ...
            unset($u);
            append_log($new_nama, 'UPDATE_PROFILE', $user['id'], "Memperbarui profil mandiri.");
        ...
        if (!password_verify($old_pw, $user['password'])) {
            ...
            append_log($user['nama'], 'UPDATE_PASSWORD', $user['id'], "Mengubah password akun.");
        ...
        if (isset($_FILES['photo']) && $_FILES['photo']['error'] === 0) {
            ...
            if (in_array($ext, $allowed)) {
                ...
                if (move_uploaded_file($_FILES['photo']['tmp_name'], $target)) {
                    ...
                    append_log($user['nama'], 'UPDATE_PHOTO', $user['id'], "Memperbarui foto profil.");
                    $_SESSION['feedback'] = ['type' => 'success', 'message' => 'Foto profil berhasil diperbarui.'];
                } else {
                    $_SESSION['feedback'] = ['type' => 'error', 'message' => 'Gagal mengunggah file ke server.'];
                }
            } else {
                $_SESSION['feedback'] = ['type' => 'error', 'message' => 'Format file tidak diizinkan. Gunakan JPG/PNG/GIF.'];
            }
        }
        header('Location: profil.php');
        exit();
